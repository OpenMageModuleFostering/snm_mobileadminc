...
;(function($, window, document, undefined) {
  ...
  Flip.prototype.init = function(flippingSide) {
    ...
    if (this.currentRatio != null) return;
  ...
  Flip.prototype.step = function(ratio) {
    ...
    if (this._isVertical()) {
      if (this.flippingSide == _FIRST_HALF) {
        ...
        $('.flipping').css(transformKey,
                           'rotateX(' + (-1 * 180 * ratio) + 'deg)');
        $('.backflipping').css(transformKey,
                          'rotateX(' + (-1 * (180 + (180 * ratio))) + 'deg)');
      ...
      if (this.flippingSide == _FIRST_HALF) {
        ...
        $('.flipping').css(transformKey,
                           'rotateY(' + (180 * ratio) + 'deg)');
        $('.backflipping').css(transformKey,
                          'rotateY(' + (180 + (180 * ratio)) + 'deg)');
    ...
    if (degree > 90) {
      ...
      // set new flipShadow
      var flipShadowRatio = Math.max(0, 1 - formula((180 - degree) / 90));
      $('.flipShadow').css('opacity', 0.9 * flipShadowRatio);

    } else {
      $('.flipping').css('zIndex', '1');
      ...
      var flipShadowRatio = Math.max(0, 1 - formula((degree) / 90));
      $('.flipShadow').css('opacity', 0.9 * flipShadowRatio);
    }
  }
  ...
  Plugin.prototype.vmousemove = function(event) {
    if (this._clickContext.downPt == null) return false;
    if (this.effect.isInAnimation) return false;
  ...
  Plugin.prototype.vmouseup = function(event) {
    if (this._clickContext.downPt == null) return true;
    if (this.effect.isInAnimation) return true;
